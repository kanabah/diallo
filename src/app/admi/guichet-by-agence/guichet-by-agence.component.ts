...
@Component({
  selector: 'app-guichet-by-agence',
  templateUrl: './guichet-by-agence.component.html',
  styleUrls: ['./guichet-by-agence.component.css']
})
export class GuichetByAgenceComponent implements OnInit {
  idUser: any;
  name: any;
  user: any;
          ...
          this.guichets = this.guichetFilters.filter(result => {
            ...
            if(result.delete == 0 &&  result.user_id._id == this.idUser && result.action != 0){
              if(this.periode.value == 'tout'){
                if(this.type.value != 'tout'){
                  if(this.action.value != 'tout'){    
                    return result.type == this.type.value && result.action == this.action.value;
                  }else{
                    return result.type == this.type.value
                  }
                }else{
                  if(this.action.value != 'tout'){    
              ...
              if(this.periode.value == 'week'){
                if(this.week.getWeekNumber(date) == this.week.getWeekNumber(createdAt) && createdAt.getFullYear() == date.getFullYear()){
                  if(this.type.value != 'tout'){
                    if(this.action.value != 'tout'){    
                      return result.type == this.type.value && result.action == this.action.value;
                    }else{
  ...
  formControl = this.fb.group({
    ...
    action: ['tout', [Validators.required]],
